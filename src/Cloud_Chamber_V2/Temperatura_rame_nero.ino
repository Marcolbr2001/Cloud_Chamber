void temperaturaramenero()
{
  ...
  {
    tft.drawLine(i*passo+X0TR,Y0TR-(float)(((float)((graficoN[i])*altGrafTR)/hgraftemp)),i*passo+X0TR-passo,Y0TR-(float)(((float)((graficoN[i-1])*altGrafTR)/hgraftemp)),colorGrafTTN);
  }
  ...
  do
  {
    ...
    control();
    tft.setTextColor(Black);
    Temp=digitalReadRameNero;

    //valore errore: non eseguire il ciclo
    ...
    if (contR==0)//Esegue un agiornamento ogni tot valori
    {
      ...
      {
        tft.drawLine((contvR-1)*passo+X0TR,Y0TR-(float)(((float)((graficoN[contvR-1])*altGrafTR)/hgraftemp)),(contvR-2)*passo+X0TR,Y0TR-(float)(((float)((graficoN[contvR-2])*altGrafTR)/hgraftemp)),colorGrafTTN);
      }
